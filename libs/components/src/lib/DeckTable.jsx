...
export const DeckTable = (props) => {
  ...
  const runQuery = () => {
    ...
    const otherIds =
      id === 'type' ? ['builder', 'eventName'] : id === 'builder' ? ['type', 'eventName'] : ['type', 'builder']
    ...
    <div className="body">
      ...
      <div className="searchWrapper">
        ...
        <div className="buttonWrapper">
          ...
          <select id="format" defaultValue={null} className="filter" onChange={(e) => setFormat(e.target.value)}>
            <option key={'All Formats'} value={''}>
              All Formats
            </option>
            {formats.map((f) => (
          ...
          <a className="searchButton desktop-only" type="submit" onClick={() => search()}>
            Search
          </a>
        </div>
      </div>
      ...
      <div id="resultsWrapper0" className="resultsWrapper0 desktop-only">
          ...
          {firstXFetched && allFetched
            ? filteredDecks.length
              ? `${decksPerPage * page - decksPerPage + 1} - ${
                  filteredDecks.length >= decksPerPage * page ? decksPerPage * page : filteredDecks.length
        ...
        <div className="buttonWrapper">
          ...
          <select id="sortSelector" defaultValue="nameASC" style={{ width: '230px' }} onChange={() => sortDecks()}>
          ...
          <a className="searchButton" type="submit" onClick={() => reset()}>
            Reset
          </a>
        </div>
      </div>
      ...
      {isTabletOrMobile ? (
        ...
        <div id="deckGalleryFlexBox">
          {filteredDecks.length ? (
            filteredDecks.slice(firstIndex, lastIndex).map((deck, index) => {
              return <DeckImage key={deck.id} index={index} deck={deck} width="360px" margin="10px 5px" padding="5px" />
            })
          ) : (
            <div />
          )}
