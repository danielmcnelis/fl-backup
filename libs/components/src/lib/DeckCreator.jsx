  ...
  return (
    <div className="admin-portal">
      <label>
        ...
        />
        <select id="builder-select" className="login" onChange={(e) => getPlayer(e.target.value)}>
          {players.map((e) => (
      ...
      <label>
        Deck Type:
        <select id="deckType" className="login" onChange={(e) => getDeckType(e.target.value)}>
      ...
      <label>
        Event:
        <select id="event" className="login" onChange={(e) => getEvent(e.target.value)}>
          <option value={null}></option>
          {events.map((e) => (
            <option value={e.name}>{e.name}</option>
          ))}
      ...
      <label>
        Display:
        <select id="display" value={display} className="login" onChange={(e) => setDisplay(e.target.value)}>
      ...
      <label>
        YDK:
        <input id="ydk" className="login" type="file" accept=".ydk" onChange={(e) => readYDK(e.target.files[0])} />
      </label>
      <a className="admin-button" type="submit" onClick={() => createDeck()}>
        Submit
      </a>
    </div>
  )
