...
export const CardTable = (props) => {
  ...
  const search = () => {
    ...
    const icons = Object.entries(iconParams)
      .filter((e) => !!e[1])
      .map((e) => e[0])
    if (icons.length) data = data.filter((d) => d.icon && icons.includes(d.icon.toLowerCase()))

    const attributes = Object.entries(attributeParams)
      .filter((e) => !!e[1])
      .map((e) => e[0])
    if (attributes.length) data = data.filter((d) => d.attribute && attributes.includes(d.attribute.toLowerCase()))

    const types = Object.entries(typeParams)
      .filter((e) => !!e[1])
      .map((e) => e[0])
    if (types.length) data = data.filter((d) => d.type && types.includes(d.type.toLowerCase()))

    if (sliders.level[0] !== 1 || sliders.level[1] !== 12)
      data = data.filter(
        (d) =>
          d.level === '?' ||
          (d.rating >= sliders.level[0] && d.rating <= sliders.level[1]) ||
          (d.level >= sliders.level[0] && d.level <= sliders.level[1])
      )
    if (sliders.atk[0] !== 0 || sliders.atk[1] !== 5000)
      data = data.filter((d) => d.atk === '?' || (d.atk >= sliders.atk[0] && d.atk <= sliders.atk[1]))
    if (sliders.def[0] !== 0 || sliders.def[1] !== 5000)
      data = data.filter((d) => d.link || d.def === '?' || (d.def >= sliders.def[0] && d.def <= sliders.def[1]))
  ...
  useEffect(() => {
    const year = format.date ? parseInt(format.date.slice(0, 4)) : 2022
  ...
  return (
    <div className="body">
      <div className="card-database-flexbox">
        <img
          src={`/assets/images/artworks/${format.icon ? `${format.icon}.jpg` : 'bls.jpg'}`}
          className="format-icon-medium desktop-only"
        />
        <div>
      ...
      {isTabletOrMobile ? (
        <div className="searchWrapper">
          ...
          <div className="query-box">
            ...
            >
              <option key="All Sets" value="">
                Sets
              </option>
              {boosters.map((b) => (
                <option key={b.id} value={b.setCode}>
                  {b.setCode}
                </option>
              ))}
            </select>
        ...
        <div className="searchWrapper">
          ...
          <select id="searchTypeSelector" defaultValue="name" className="filter" onChange={() => runQuery()}>
          ...
          {formatName ? (
            ...
            <select id="format" defaultValue="" className="filter" onChange={(e) => updateFormat(e)}>
              <option key="Current" value="">
                Current
              </option>
              {formats
          ...
          <select id="booster" defaultValue="" className="filter" onChange={(e) => setBooster(e.target.value)}>
            <option key="All Sets" value="">
              All Sets
            </option>
            {boosters.map((b) => (
      ...
      {!advanced ? (
        <div className="refinedWrapper">
          <a className="refinedButton" type="submit" onClick={() => setAdvanced(!advanced)}>
      ...
      <div id="resultsWrapper0" className="resultsWrapper0">
          ...
          {firstXFetched && allFetched
            ? filteredCards.length
              ? `${cardsPerPage * page - cardsPerPage + 1} - ${
                  filteredCards.length >= cardsPerPage * page ? cardsPerPage * page : filteredCards.length
        ...
        <div className="buttonWrapper">
          ...
          <a className="searchButton desktop-only" type="submit" onClick={() => reset()}>
            Reset
          </a>
        </div>
      </div>
            ...
            <tbody>
              {filteredCards.length ? (
                filteredCards.slice(firstIndex, lastIndex).map((card, index) => {
                  if (isTabletOrMobile) {
                    return (
                      <MobileCardRow key={card.id} index={index} card={card} status={banlist[card.id.toString()]} />
                    )
                  } else {
