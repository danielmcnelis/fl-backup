...
export const PlayerProfile = (props = {}) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const discriminator = props.location.hash.slice(-4)
				const headers = { discriminator: discriminator }
				const { data } = await axios.get(`/api/players/${props.match.params.id}`, { headers })
				setPlayer(data)
			} catch (err) {
				console.log(err)
				setPlayer(null)
