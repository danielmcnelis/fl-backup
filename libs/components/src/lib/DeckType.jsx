...
export const DeckType = (props) => {
  ...
  const categoryImage =
    summary.deckCategory === 'Aggro'
      ? Helmet
      : summary.deckCategory === 'Combo'
      ? Controller
      : summary.deckCategory === 'Control'
      ? Orb
      : summary.deckCategory === 'Lockdown'
      ? Lock
      : Thinking
        ...
        <tbody>
          <tr className="single-deck-info-1">
            <td>
              <div onClick={() => goToFormat()} className="single-deck-cell">
                <div className="single-deck-format-link" style={{ paddingRight: '7px' }}>
                  <b>Format:</b> {summary.format.name}
                </div>
            ...
            <td>
              <div className="single-deck-cell">
                <div className="single-deck-category" style={{ paddingRight: '7px' }}>
                  <b>Category:</b> {summary.deckCategory}
                </div>
            ...
            <td>
              <div className="single-deck-cell">
                <div className="single-deck-category" style={{ paddingRight: '7px' }}>
                  <b>Frequency:</b> {summary.percent}%
                </div>
                <img
                  className="single-deck-category-emoji"
                  style={{ width: '28px' }}
                  src={`/assets/images/emojis/math.png`}
                />
              </div>
            </td>
          </tr>
          ...
          {summary.mainMonsters.map((data, index) => {
            const info =
              data['1'] > data['2'] && data['1'] > data['3']
                ? `1x in ${Math.round((data['1'] / summary.analyzed) * 100)}%`
                : data['2'] >= data['1'] && data['2'] >= data['3']
                ? `2x in ${Math.round((data['2'] / summary.analyzed) * 100)}%`
                : `3x in ${Math.round((data['3'] / summary.analyzed) * 100)}%`

            const details =
              (data['3'] ? `3x in ${Math.round((data['3'] / summary.analyzed) * 100)}%\n` : '') +
              (data['2'] ? `2x in ${Math.round((data['2'] / summary.analyzed) * 100)}%\n` : '') +
              (data['1'] ? `1x in ${Math.round((data['1'] / summary.analyzed) * 100)}%\n` : '') +
              (summary.analyzed - data.decks
                ? `0x in ${Math.round(((summary.analyzed - data.decks) / summary.analyzed) * 100)}%`
                : '')

            return (
              <div className="popular-main" key={'m' + data.card.ypdId}>
                <CardImage
                  className="popular-main-card"
                  width="72px"
                  padding="1px"
                  margin="0px"
                  card={data.card}
                  status={banlist[data.card.id]}
                />
                <div onClick={() => toggle('main-monsters', index)}>
                  <div id={'main-monsters-info-' + index} className="deckType-info">
                    {info}
                  </div>
                  <div id={'main-monsters-details-' + index} className="expanded-info">
                    {details}
                  </div>
                </div>
              </div>
            ...
            return (
              <div className="popular-main" key={'m' + data.card.ypdId}>
                ...
                <div onClick={() => toggle('main-spells', index)}>
                  <div id={'main-spells-info-' + index} className="deckType-info">
                    {info}
                  </div>
                  <div id={'main-spells-details-' + index} className="expanded-info">
                    {details}
                  </div>
                </div>
              </div>
            ...
            return (
              <div className="popular-main" key={'m' + data.card.ypdId}>
                ...
                <div onClick={() => toggle('main-traps', index)}>
                  <div id={'main-traps-info-' + index} className="deckType-info">
                    {info}
                  </div>
                  <div id={'main-traps-details-' + index} className="expanded-info">
                    {details}
                  </div>
                </div>
              </div>
                ...
                return (
                  <div className="popular-side" key={'e' + data.card.ypdId}>
                    ...
                    <div onClick={() => toggle('extra', index)}>
                      <div id={'extra-info-' + index} className="deckType-info">
                        {info}
                      </div>
                      <div id={'extra-details-' + index} className="expanded-info">
                        {details}
                      </div>
                    </div>
                  </div>
            ...
            return (
              <div className="popular-side" key={'s' + data.card.ypdId}>
                ...
                <div onClick={() => toggle('side-monsters', index)}>
                  <div id={'side-monsters-info-' + index} className="deckType-info">
                    {info}
                  </div>
                  <div id={'side-monsters-details-' + index} className="expanded-info">
                    {details}
                  </div>
                </div>
              </div>
            ...
            return (
              <div className="popular-side" key={'s' + data.card.ypdId}>
                ...
                <div onClick={() => toggle('side-spells', index)}>
                  <div id={'side-spells-info-' + index} className="deckType-info">
                    {info}
                  </div>
                  <div id={'side-spells-details-' + index} className="expanded-info">
                    {details}
                  </div>
                </div>
              </div>
            ...
            return (
              <div className="popular-side" key={'s' + data.card.ypdId}>
                ...
                <div onClick={() => toggle('side-traps', index)}>
                  <div id={'side-traps-info-' + index} className="deckType-info">
                    {info}
                  </div>
                  <div id={'side-traps-details-' + index} className="expanded-info">
                    {details}
                  </div>
                </div>
              </div>
