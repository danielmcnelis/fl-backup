...
export const DeckTypeCreator = () => {
  ...
  return (
    <div className="admin-portal">
      <label>
        Name:
        <input id="name" className="login" type="text" onChange={(e) => setName(e.target.value)} />
      </label>

      <label>
        Category:
        <select id="category" className="login" onChange={(e) => setCategory(e.target.value)}>
      ...
      <label>
        ...
        <input
          ...
          type="search"
          onKeyDown={(e) => {
            if (e.key === 'Enter') findCards(e.target.value, 'left')
          }}
        />
        <select id="left-card-select" className="login" onChange={(e) => getCard(e.target.value, 'left')}>
          {leftCards.map((e) => (
      ...
      <label>
        ...
        <input
          ...
          type="search"
          onKeyDown={(e) => {
            if (e.key === 'Enter') findCards(e.target.value, 'center')
          }}
        />
        <select id="center-card-select" className="login" onChange={(e) => getCard(e.target.value, 'center')}>
          {centerCards.map((e) => (
            <option value={e.name}>{e.name}</option>
          ))}
        </select>
      ...
      <label>
        ...
        <input
          ...
          type="search"
          onKeyDown={(e) => {
            if (e.key === 'Enter') findCards(e.target.value, 'right')
          }}
        />
        <select id="right-card-select" className="login" onChange={(e) => getCard(e.target.value, 'right')}>
          {rightCards.map((e) => (
            <option value={e.name}>{e.name}</option>
          ))}
        </select>
      ...
      <a className="admin-button" type="submit" onClick={() => create()}>
        Submit
      </a>
    </div>
  )
