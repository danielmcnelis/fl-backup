...
export const PrintRow = (props) => {
  ...
  const tcgPlayerUrl =
    print.tcgPlayerUrl + '?utm_campaign=affiliate&utm_medium=FormatLibrary&utm_source=FormatLibrary' ||
    `https://store.tcgplayer.com/yugioh/${urlize(print.setName)}/${urlize(
      print.cardName
    )}?utm_campaign=affiliate&utm_medium=FormatLibrary&utm_source=FormatLibrary`
  ...
  return (
    <tr onClick={() => openNewTab()} className={`${evenOrOdd}-print-row`}>
      ...
      <td className="print-cell-2">{print.rarity}</td>
      <td className="print-cell-3">{print.cardCode}</td>
      <td className="print-cell-4">{print.setName}</td>
